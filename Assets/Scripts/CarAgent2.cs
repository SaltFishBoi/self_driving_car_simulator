...
public class CarAgent2 : Agent
{
    ...
    public override void CollectObservations(VectorSensor vectorSensor)
    {
        ...
        vectorSensor.AddObservation(ObserveRay(0f, 0.75f, (float)Math.Atan(0.75f / 0f) * (180f / ((float)Math.PI)), true));
        vectorSensor.AddObservation(ObserveRay(1.5f, 0.75f, (float)Math.Atan(0.75f / 1.5f) * (180f / ((float)Math.PI)), true));
        vectorSensor.AddObservation(ObserveRay(1.5f, 0.5f, (float)Math.Atan(0.5f / 1.5f) * (180f / ((float)Math.PI)), true));
        ...
        vectorSensor.AddObservation(ObserveRay(1.5f, -0.75f, (float)Math.Atan(-0.75f / 1.5f) * (180f / ((float)Math.PI)), true));
        vectorSensor.AddObservation(ObserveRay(0f, -0.75f, (float)Math.Atan(-0.75f / 0f) * (180f / ((float)Math.PI)), true));
        vectorSensor.AddObservation(ObserveRay(-1.5f, 0f, (float)Math.Atan(-0f / 1.5f) * (180f / ((float)Math.PI)) - 180f, true));
    }
    ...
    private float ObserveRay(float z, float x, float angle, bool dbug)
    {
        ...
        if (hit.transform != null) // good hit
        {
            rettag = hit.transform.tag;
            retval = hit.distance / RAY_DIST;
            if (dbug == true)
            {
                Debug.DrawRay(position, dir * hit.distance, Color.yellow);
            }
